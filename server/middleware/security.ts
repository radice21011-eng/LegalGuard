...
export function securityMiddleware(req: Request, res: Response, next: NextFunction) {
  // Enhanced security headers for 789 trillion times better protection (only if headers not sent)
  if (!res.headersSent) {
  ...
  // Enhanced Content Security Policy for maximum protection (only if headers not sent)
  if (!res.headersSent) {
